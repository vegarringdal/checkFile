...
--------------------------
	select 
		...
		count(tag_no) as No,
		CASE
		WHEN tag_eng_code = "E" or tag_eng_code = "C" or tag_contractor is null THEN "Must be corrected"
  		ELSE "INFO"
  		END,
...
--------------------------
	select 
		...
		count(tag_no) as No,
		CASE 
		WHEN (tag_discipline != "E" and tag_discipline != "I" and tag_discipline != "T") or tag_contractor is null or tag_discipline is null THEN "Must be corrected"
  		ELSE "INFO"
  		END,
		ifnull(tag_contractor, 'All') as Tag_contractor,
...
--------------------------
	select 
		...
		count(tag_no) as No,
		CASE 
		WHEN (tag_cable_status != "RE" and tag_cable_status != "CR" and tag_cable_status != "RO") or tag_contractor is null or tag_cable_status is null THEN "Must be corrected"
  		ELSE "INFO"
  		END,
		ifnull(tag_contractor, 'All') as Tag_contractor,
		ifnull(tag_discipline, 'All') as Discipline,
		'Cable status - Eng Code A, B' as Groups,
		'' as 'Last Week',
		'' as 'Comment meeting'
	from 
		be_tags_sub_AB
...
--------------------------
	select 
		...
		count(tag_no) as No,
		CASE 
		WHEN tag_contractor is null or tag_installer_contractor is null THEN "Must be corrected"
  		ELSE "INFO"
  		END,
		ifnull(tag_contractor, 'All') as Tag_contractor,
		ifnull(tag_discipline, 'All') as Discipline,
...
--------------------------
	select 
		...
		OR tag_cable_length = '0' OR tag_cable_length = 0 or tag_cable_length is null then 'Must be corrected'
		else 'INFO'
		end AS TARGET,
		ifnull(tag_contractor, 'All') as Tag_contractor,
		ifnull(tag_discipline, 'All') as Discipline,
...
--------------------------
	select 
		...
		case 
		when tag_contractor is null OR tag_discipline is null then 'Must be corrected'
		else 'INFO'
		end AS TARGET,
		ifnull(tag_contractor, 'All') as Tag_contractor,
		ifnull(tag_discipline, 'All') as Discipline,
